...
function renderCampaignDetail() {
  ...
  if (els.campaignUnitsGrid) {
    els.campaignUnitsGrid.innerHTML = `<div class="campaign-card-grid">${units.map(u => `<article class="campaign-unit-card"><div class="headerUnidad"><div><div class="topbar-kicker">${escapeHtml(data.group.empresa)}</div><h4>${escapeHtml(u.numeroEconomico)}</h4><div class="muted">${escapeHtml(u.marca || '')} · ${escapeHtml(u.modelo || '')}</div></div><span class="badge badge-info">${escapeHtml(data.group.nombre)}</span></div><div class="fleet-bus-wrap campaign-fleet-bus-wrap">${campaignBusMarkup(u)}<div class="mini-grid campaign-info-grid" style="flex:1"><div><span class="label">Obra</span><strong>${escapeHtml(u.numeroObra || '—')}</strong></div><div><span class="label">Reportes</span><strong>${Number(u.reportesCount||0)}</strong></div><div><span class="label">Último movimiento</span><strong>${escapeHtml(u.lastReportAt ? fmtDate(u.lastReportAt) : 'Sin movimiento')}</strong></div><div><span class="label">Campaña</span><strong>${escapeHtml(data.group.nombre || '—')}</strong></div></div></div><div class="campaign-status ${campaignStatusClass(u.status)}"><strong>${campaignStatusLabel(u.status)}</strong></div><div class="small muted">${escapeHtml(u.notas || 'Sin notas')}</div>${(u.evidencia||[]).length ? buildImageGallery(u.evidencia, '') : '<div class="muted">Sin evidencia cargada.</div>'}<div class="stack-inline">${isRole('admin') ? `<button class="btn btn-secondary" type="button" data-campaign-edit="${u.id}">Editar</button><button class="btn btn-ghost" type="button" data-campaign-delete="${u.id}">Eliminar</button>` : ''}${u.fleetUnitId ? `<button class="btn btn-ghost" type="button" data-campaign-fleet="${u.fleetUnitId}">Ver unidad</button>` : ''}</div></article>`).join('')}</div>`;
